...
router.get('/', async (req, res, next) => {
    try {
        const designation = await Designation.find().populate('employees');
        designation && res.status(200).json({ message: "Success", designation })
    } catch (error) {
        next(error)
    }
})
...
//add a department
router.post('/adddesignation', async (req, res, next) => {
    try {
        const designationdata = new Designation({
            title: req.body.title,
            // description: req.body.description
        })
        const designation = await designationdata.save();
        designation && res.status(200).json({ message: "Added Designation", designation })
    } catch (err) {
        next(err)
